class Postcat < Formula
  ...
  # Postfix, so this formula also installs a `pcat` copy of the same binary
  # for anyone who wants an unambiguous name on their PATH.
  ...
  def post_install
    ...
    # restore the normal Homebrew executable permissions afterwards. Both
    # `postcat` and `pcat` are separate files here, so clear each one.
    [bin/"postcat", bin/"pcat"].each do |exe|
      next unless exe.exist?

      exe.chmod 0755
      Kernel.system "/usr/bin/xattr", "-d", "com.apple.quarantine", exe.to_s,
